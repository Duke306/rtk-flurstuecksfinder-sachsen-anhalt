...
def parcel(pid):
    ...
    load_combined = getattr(parcels, 'parcel_and_geojson', None)
    if callable(load_combined):
        p, gj = load_combined(pid)
    else:
        # Support older installed helpers when only app.py was updated.
        p = parcels.get(pid, False)
        gj = parcels.parcel_geojson(pid) if p else None
    if not p or not gj:
...
<title>RTK Flurstücksfinder Sachsen-Anhalt v6.1.1</title><link rel="stylesheet" href="/static/maplibre-gl.css">
<style>
...
.navminihead{display:flex;justify-content:space-between;gap:10px;align-items:center;margin-bottom:8px}.navminiwrap{height:245px;position:relative;border-radius:9px;overflow:hidden;background:#222}.navmini{height:100%;width:100%}.navnorth{position:absolute;top:8px;right:8px;z-index:5;background:#111d;border:1px solid #777;border-radius:7px;padding:5px 8px;font-weight:800;font-size:15px;pointer-events:none}.navcourse{margin-top:8px;text-align:center;font-size:14px;line-height:1.5}.navcourse b{font-size:16px}.mini-pos{position:relative;width:34px;height:34px}.mini-pos .head{position:absolute;left:8px;top:-11px;width:18px;height:20px;text-align:center;font-size:20px;line-height:20px;color:#64a9ff;text-shadow:0 1px 3px #000;transform-origin:50% 20px}.mini-pos .dot{position:absolute;left:8px;top:8px;width:18px;height:18px;border-radius:50%;background:#1d7cff;border:3px solid #fff;box-shadow:0 1px 5px #000}.mini-target{width:24px;height:24px;border-radius:50%;background:#e53935;border:3px solid #fff;box-shadow:0 1px 5px #000}.navhint{font-size:11px;color:#aaa;text-align:center;margin-top:5px}
</style></head><body>
<div class="top"><div class="title">RTK Flurstücksfinder · Sachsen-Anhalt <span class="muted">v6.1.1</span></div><div class="checks"><div class="check">WLAN<br><b id="wifi">--</b></div><div class="check">Internet<br><b id="inet">--</b></div><div class="check">SAPOS<br><b id="sapos">--</b></div><div class="check">RTK<br><b id="rtk">--</b></div></div></div>
...
<div id="nav" class="pane active"><div class="card current"><b>Aktuelle Position</b><div id="autoparcel" class="info">Warte auf GNSS…</div></div><div class="card"><div id="targetname" class="center">Kein Ziel gewählt</div><div id="distance" class="big">--</div><div id="navarrow" class="arrow">↑</div><div id="navbearing" class="center muted">Zielrichtung: --</div><div id="offset" class="center" style="margin-top:4px">--</div></div><div class="card"><div class="navminihead"><b>Navigationskarte</b><span class="badge">NORD OBEN</span></div><div class="navminiwrap"><div id="navmini" class="navmini"></div><div class="navnorth">N ↑</div></div><div id="navcourse" class="navcourse">Warte auf Position…</div><div class="navhint">Blau = aktuelle Position · Rot = Ziel · Pfeil am blauen Punkt = Bewegungsrichtung</div></div><div class="card"><button class="primary" onclick="nearestCurrentBoundary()">Nächsten Punkt der ausgewählten Grenze ansteuern</button></div></div>
...
let selectedParcel=null,targets=[],target=null,map=null,parcelFeature=null,currentMarker=null,targetMarker=null,lastNearE=null,lastNearN=null,targetLabelMarkers=[],heightMarkerObjects=[],heightData=null,lastStatus=null,heightTimer=null,heightMeasuring=false,mapReady=null,miniMap=null,miniCurrentMarker=null,miniTargetMarker=null,miniMoveE=null,miniMoveN=null,miniMoveBearing=null;
function tab(id,b){document.querySelectorAll('.pane').forEach(x=>x.classList.remove('active'));document.querySelectorAll('.tabs button').forEach(x=>x.classList.remove('active'));document.getElementById(id).classList.add('active');b.classList.add('active');if(id==='mapPane'){initMap();setTimeout(()=>map&&map.resize(),50)}if(id==='nav'){initMiniMap();setTimeout(()=>miniMap&&miniMap.resize(),50)}if(id==='heightPane')loadHeights()}
...
function compassName(b){if(b==null||!isFinite(b))return'--';let n=['N','NE','E','SE','S','SW','W','NW'];return n[Math.round((((b%360)+360)%360)/45)%8]}
function miniZoom(d){if(d==null||!isFinite(d))return 18;if(d<=1.5)return 21;if(d<=3)return 20.5;if(d<=7)return 20;if(d<=15)return 19.5;if(d<=35)return 19;if(d<=80)return 18;if(d<=180)return 17;return 16}
function initMiniMap(){if(miniMap||typeof maplibregl==='undefined')return;miniMap=new maplibregl.Map({container:'navmini',style:'/api/map/style',center:[11.7,51.9],zoom:8,maxZoom:21,bearing:0,pitch:0,interactive:false,attributionControl:false});miniMap.addControl(new maplibregl.AttributionControl({compact:true}),'bottom-right');miniMap.on('load',()=>{miniMap.addSource('mini-route',{type:'geojson',data:{type:'FeatureCollection',features:[]}});miniMap.addLayer({id:'mini-route-line',type:'line',source:'mini-route',paint:{'line-color':'#e53935','line-width':4,'line-dasharray':[1.5,1]}});miniMap.addSource('mini-parcel',{type:'geojson',data:{type:'FeatureCollection',features:[]}});miniMap.addLayer({id:'mini-parcel-fill',type:'fill',source:'mini-parcel',paint:{'fill-color':'#ffcc00','fill-opacity':0.12}});miniMap.addLayer({id:'mini-parcel-line',type:'line',source:'mini-parcel',paint:{'line-color':'#ff9800','line-width':3}});drawMiniParcel();if(lastStatus)updateMiniNav(lastStatus)});miniMap.on('error',()=>{})}
function drawMiniParcel(){if(!miniMap||!miniMap.isStyleLoaded()||!miniMap.getSource('mini-parcel'))return;miniMap.getSource('mini-parcel').setData(parcelFeature||{type:'FeatureCollection',features:[]})}
function updateMiniNav(s){if(!s)return;initMiniMap();if(s.lon!=null&&s.lat!=null&&s.E!=null&&s.N!=null){if(miniMoveE==null){miniMoveE=s.E;miniMoveN=s.N}else{let de=s.E-miniMoveE,dn=s.N-miniMoveN;if(Math.hypot(de,dn)>=0.20){miniMoveBearing=(Math.atan2(de,dn)*180/Math.PI+360)%360;miniMoveE=s.E;miniMoveN=s.N}}}let targetBearing=s.nav?s.nav.bearing:null;if(s.nav){navbearing.textContent=`Zielrichtung: ${targetBearing.toFixed(0).padStart(3,'0')}° · ${compassName(targetBearing)}`}else navbearing.textContent='Zielrichtung: --';let moveTxt=miniMoveBearing==null?'Bewegung: --':`Bewegung: ${miniMoveBearing.toFixed(0).padStart(3,'0')}° · ${compassName(miniMoveBearing)}`;let targetTxt=s.nav?`Ziel: ${targetBearing.toFixed(0).padStart(3,'0')}° · ${compassName(targetBearing)} · ${fmtD(s.nav.distance)}`:'Kein Ziel gewählt';navcourse.innerHTML=`<b>${targetTxt}</b><br>${moveTxt}`;if(!miniMap||!miniMap.isStyleLoaded()||s.lon==null||s.lat==null)return;if(!miniCurrentMarker){let el=document.createElement('div');el.className='mini-pos';el.innerHTML='<div class="head">▲</div><div class="dot"></div>';miniCurrentMarker=new maplibregl.Marker({element:el,anchor:'center'}).setLngLat([s.lon,s.lat]).addTo(miniMap)}else miniCurrentMarker.setLngLat([s.lon,s.lat]);let head=miniCurrentMarker.getElement().querySelector('.head');head.style.display=miniMoveBearing==null?'none':'block';if(miniMoveBearing!=null)head.style.transform=`rotate(${miniMoveBearing}deg)`;let route={type:'FeatureCollection',features:[]};if(target&&target.lon!=null&&target.lat!=null&&s.nav){route.features=[{type:'Feature',properties:{},geometry:{type:'LineString',coordinates:[[s.lon,s.lat],[target.lon,target.lat]]}}];if(!miniTargetMarker){let el=document.createElement('div');el.className='mini-target';miniTargetMarker=new maplibregl.Marker({element:el,anchor:'center'}).setLngLat([target.lon,target.lat]).addTo(miniMap)}else miniTargetMarker.setLngLat([target.lon,target.lat])}else if(miniTargetMarker){miniTargetMarker.remove();miniTargetMarker=null}let src=miniMap.getSource('mini-route');if(src)src.setData(route);miniMap.easeTo({center:[s.lon,s.lat],zoom:miniZoom(s.nav?s.nav.distance:null),bearing:0,pitch:0,duration:350})}
...
initMiniMap();loadHeights();loadCounties();
</script></body></html>'''
